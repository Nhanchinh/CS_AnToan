...
function getNkNr(keyLength) {
    switch (keyLength) {
        case 16: return { Nk: 4, Nr: 10 }; // AES-128
        case 24: return { Nk: 6, Nr: 12 }; // AES-192
        case 32: return { Nk: 8, Nr: 14 }; // AES-256
        default: throw new Error("Unsupported key length. Use 16, 24, or 32 bytes.");
    }
}
...
// Key Expansion - Expand the key into the key schedule
function keyExpansion(key) {
    const { Nk, Nr } = getNkNr(key.length);
    let w = new Array(Nb * (Nr + 1));
    let temp = new Array(4);
...
// AES Encryption - Encrypt a 16-byte block
function encryptBlock(input, w, Nr) {
    let state = new Array(4);
    for (let i = 0; i < 4; i++) {
        state[i] = new Array(Nb);
    }
...
// AES Decryption - Decrypt a 16-byte block
function decryptBlock(input, w, Nr) {
    let state = new Array(4);
    for (let i = 0; i < 4; i++) {
        state[i] = new Array(Nb);
    }
...
function encrypt(input, key) {
    ...
    if (![16, 24, 32].includes(keyBytes.length)) {
        throw new Error("Key length must be 16, 24, or 32 bytes for AES-128, AES-192, or AES-256.");
    }
    ...
    for (let i = 0; i < inputBytes.length; i += 16) {
        const block = inputBytes.slice(i, i + 16);
        const encryptedBlock = encryptBlock(block, w, Nr);
        output.set(encryptedBlock, i);
    }
...
function decrypt(input, key) {
    try {
        ...
        for (let i = 0; i < encryptedBytes.length; i += 16) {
            const block = encryptedBytes.slice(i, i + 16);
            const decryptedBlock = decryptBlock(block, w, Nr);
            output.set(decryptedBlock, i);
        }
...
if (typeof window === 'undefined') {
    // Test AES-128
    const key128 = "MySecretKey12345"; // 16 bytes
    const plaintext = "Xin chào, đây là tiếng Việt!";
    console.log("--- AES-128 ---");
    const encrypted128 = encrypt(plaintext, key128);
    console.log("Encrypted:", encrypted128);
    const decrypted128 = decrypt(encrypted128, key128);
    console.log("Decrypted:", decrypted128);

    // Test AES-192
    const key192 = "MySecretKey123456789012"; // 24 bytes
    console.log("\n--- AES-192 ---");
    const encrypted192 = encrypt(plaintext, key192);
    console.log("Encrypted:", encrypted192);
    const decrypted192 = decrypt(encrypted192, key192);
    console.log("Decrypted:", decrypted192);

    // Test AES-256
    const key256 = "MySecretKey12345678901234567890"; // 32 bytes
    console.log("\n--- AES-256 ---");
    const encrypted256 = encrypt(plaintext, key256);
    console.log("Encrypted:", encrypted256);
    const decrypted256 = decrypt(encrypted256, key256);
